...
const Profile = () => {
  ...
  const handleAvatarUpload = async (e) => {
    ...
    try {
      await axios.post("/api/profile/avatar/update", formData, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
          "Content-Type": "multipart/form-data",
        },
  ...
  const handleAvatarDelete = async () => {
    try {
      await axios.delete("/api/profile/avatar/delete", {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
        ...
        <section className="avatar-section">
          ...
          {user.avatar ? (
            <div className="avatar-display">
              ...
              <button onClick={handleAvatarDelete} className="btn btn-danger">
                <i className="fas fa-trash"></i> Delete
              </button>
            </div>
          ) : (
            <div className="avatar-upload-section">
              <label htmlFor="avatar-upload" className="btn btn-info">
      ...
      <section className="profile-info-section">
        {editing ? (
          ...
          <div className="profile-info">
            <p>
              <i className="fas fa-user"></i> <b>First Name:</b>{" "}
              {user.firstName}
            </p>
            <p>
              <i className="fas fa-user"></i> <b>Second Name:</b>{" "}
              {user.secondName}
            </p>
            <p>
              <i className="fas fa-user-tag"></i> <b>Username:</b>{" "}
              {user.username}
            </p>
            <p>
            ...
            <p>
              <i className="fas fa-map-marker-alt"></i> <b>Location:</b>{" "}
              {user.location}
            </p>
            <p>
            ...
            {user.company && (
              <p>
                <i className="fas fa-building"></i> <b>Company:</b>{" "}
                {user.company}
              </p>
            )}
        ...
        <ul className="activity-list">
          {user.sentMessages && user.sentMessages.length > 0 ? (
            user.sentMessages.map((message) => (
              <li
                key={message.id}
                onClick={() => handleSentMessageClick(message.id)}
              >
        ...
        <ul className="activity-list">
          {user.receivedMessages && user.receivedMessages.length > 0 ? (
            user.receivedMessages.map((message) => (
              <li
                key={message.id}
                onClick={() => handleReceivedMessageClick(message.id)}
              >
